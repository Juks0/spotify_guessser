// src/lib/routers/playbackRoute.ts
...
router.get('/playback', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const url = `https://api.spotify.com/v1/me/player${queryParams ? '?' + queryParams : ''}`;
        console.log(url);
...
router.put('/playback/play', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const url = `https://api.spotify.com/v1/me/player/play${queryParams ? '?' + queryParams : ''}`;
        const response = await fetch(url, {
            method: 'PUT',
            headers: {
                'Authorization': 'Bearer ' + access_token,
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Playback started' });
        }
        ...
        res.status(204).json({ message: 'Playback started' });
...
router.put('/playback/pause', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const url = `https://api.spotify.com/v1/me/player/pause${queryParams ? '?' + queryParams : ''}`;
        const response = await fetch(url, {
            method: 'PUT',
            headers: {
                'Authorization': 'Bearer ' + access_token,
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Playback paused' });
        }
        ...
        res.status(204).json({ message: 'Playback paused' });
...
router.post('/playback/next', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const url = `https://api.spotify.com/v1/me/player/next${queryParams ? '?' + queryParams : ''}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + access_token,
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Skipped to next track' });
        }
        ...
        res.status(204).json({ message: 'Skipped to next track' });
...
router.post('/playback/previous', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const url = `https://api.spotify.com/v1/me/player/previous${queryParams ? '?' + queryParams : ''}`;
        const response = await fetch(url, {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + access_token,
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Skipped to previous track' });
        }
        ...
        res.status(204).json({ message: 'Skipped to previous track' });
...
router.put('/playback/seek', async (req: Request, res: Response) => {
    ...
    if (!position_ms || isNaN(Number(position_ms)) || Number(position_ms) < 0) {
    ...
    try {
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Seeked to position' });
        }
        ...
        res.status(204).json({ message: 'Seeked to position' });
...
router.put('/playback/repeat', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: `Repeat mode set to ${state}` });
        }
        ...
        res.status(204).json({ message: `Repeat mode set to ${state}` });
...
router.put('/playback/shuffle', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: `Shuffle ${state === 'true' ? 'enabled' : 'disabled'}` });
        }
        ...
        res.status(204).json({ message: `Shuffle ${state === 'true' ? 'enabled' : 'disabled'}` });
...
router.put('/playback/volume', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: `Volume set to ${volume}%` });
        }
        ...
        res.status(204).json({ message: `Volume set to ${volume}%` });
...
router.post('/playback/queue', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (response.status === 204) {
            return res.status(204).json({ message: 'Added to queue' });
        }
        ...
        res.status(204).json({ message: 'Added to queue' });
